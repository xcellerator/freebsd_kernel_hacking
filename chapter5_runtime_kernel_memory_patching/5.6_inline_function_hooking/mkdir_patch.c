/* Compile with the -lkvm GCC flag */
...
int main (int argc, char *argv[])
{
	...
	kvm_t *kd;
	struct nlist nl[] = { {NULL}, {NULL}, {NULL}, {NULL}, {NULL}, {NULL}, {NULL}, };
	...
	/* Search through mkdir for the call to kern_mkdirat */
	for ( i = 0 ; i < sizeof(kmalloc) ; i++ )
	{
		if ( mkdir_code[i] == 0xeb )
		{
			call_offset = i;
			break;
		}
	...
	*(unsigned int *)&kmalloc[217] = nl[4].n_value - (nl[0].n_value + OFFSET_COPYOUT);
	...
	/* Overwrite sys_mkdir with kmalloc */
	...
	printf("[>] Allocated 0x%x bytes of kernel memory at 0x%x\n", size, (long)addr);
